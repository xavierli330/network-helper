...
// defaultSoul is the built-in personality, boundaries and tone definition.
...
// defaultIdentity is the built-in name, emoji and introduction.
...
// defaultTools is the built-in tool usage guidance.
const defaultTools = `# 工具使用指南
...
func EnsureDefaultFiles(dataDir string) {
...
func New(router *llm.Router, registry *Registry, embedder llm.Embedder, db *store.DB, opts ...AgentOptions) *Agent {
	...
	if opt.DataDir != "" {
		EnsureDefaultFiles(opt.DataDir)
