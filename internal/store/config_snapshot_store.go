...
func (db *DB) InsertConfigSnapshot(cs model.ConfigSnapshot) (int, error) {
	result, err := db.Exec(`INSERT INTO config_snapshots (device_id, config_text, diff_from_prev, source_file) VALUES (?, ?, ?, ?)`,
		cs.DeviceID, cs.ConfigText, cs.DiffFromPrev, cs.SourceFile)
	if err != nil {
		return 0, err
	}
